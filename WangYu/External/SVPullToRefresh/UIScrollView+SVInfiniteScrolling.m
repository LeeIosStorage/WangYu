...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        [self setCustomView:nil forState:SVInfiniteScrollingStateAll];
    }
...
#pragma mark - Setters
static int customViewLabel_Tag = 10201;
- (void)setCustomView:(UIView *)view forState:(SVInfiniteScrollingState)state {
    id viewPlaceholder = view;
    
    if(!viewPlaceholder){
        
        UIView *customView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 200, 20)];
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 200, 20)];
        label.textColor = SKIN_TEXT_COLOR2;
        label.font = SKIN_FONT_FROMNAME(13);
        label.backgroundColor = [UIColor clearColor];
        label.numberOfLines = 1;
        label.textAlignment = NSTextAlignmentCenter;
        label.tag = customViewLabel_Tag;
//        label.text = @"上拉加载更多";
        [customView addSubview:label];
        [self addSubview:customView];
        viewPlaceholder = customView;
    }
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if(hasCustomView) {
        ...
        [self.activityIndicatorView setFrame:CGRectMake(origin.x + 40, origin.y, self.activityIndicatorView.frame.size.width, self.activityIndicatorView.frame.size.height)];
        
        UIView *customLoadView = (UIView*)customView;
        UILabel *loadLabel = (UILabel *)[customLoadView viewWithTag:customViewLabel_Tag];
        switch (newState) {
